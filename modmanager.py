...
class ModManager(QMainWindow):
    def __init__(self):
        ...
        self.last_folder = os.getcwd()  # Remember last folder used
    ...
    def show_welcome(self):
        ...
        # Load pixel font for main message
        pixel_font_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "pixel_font.ttf")
        pixel_id = QFontDatabase.addApplicationFont(pixel_font_path)
        pixel_family = QFontDatabase.applicationFontFamilies(pixel_id)[0] if pixel_id != -1 else "Arial"
        main_font = QFont(pixel_family, 14)  # just bigger, no bold

        # Italic font for last line
        italic_font = QFont(pixel_family, 12)
        italic_font.setItalic(True)

        # Main message
        main_text = (
            "Hi, it's Homer, the new workshop manager.\n\n"
            "I was told you're the one to speak to about client folders? Some joker hid our music vinyl there, and this place needs some ambiance...\n\n"
            "Apparently the place to look for is somewhere in the common area, past the steam room. Don't keep me waiting. *CLICK*\n\n"
        )
        last_line = "(Select the hlm2_music_desktop.wad in your HM2 installation folder, typically in /Steam/steamapps/common/Hotline Miami 2/)"
    ...
    def add_mod_row(self, mod_name, patch_files, music_file):
        ...
        font_name.setPointSize(11)  # slightly larger
        font_name.setBold(True)
        item.setFont(1, font_name)

        font_active = item.font(0)
        font_active.setPointSize(16)  # bigger tick
